...
/**
 ...
 For others (macOS/watchOS or iOS/tvOS 10-), these method use the `TXAdImageGraphics.h` to implements the same behavior (but without dynamic bitmap support)
*/

/// A closure for drawing an image.
typedef void (^TXAdGraphicsImageDrawingActions)(CGContextRef _Nonnull context);
/// Constants that specify the color range of the image renderer context.
typedef NS_ENUM(NSInteger, TXAdGraphicsImageRendererFormatRange) {
    /// The image renderer context doesn’t specify a color range.
    TXAdGraphicsImageRendererFormatRangeUnspecified = -1,
    /// The system automatically chooses the image renderer context’s pixel format according to the color range of its content.
    TXAdGraphicsImageRendererFormatRangeAutomatic = 0,
    /// The image renderer context supports wide color.
    TXAdGraphicsImageRendererFormatRangeExtended,
    /// The image renderer context doesn’t support extended colors.
    TXAdGraphicsImageRendererFormatRangeStandard
};

/// A set of drawing attributes that represent the configuration of an image renderer context.
@interface TXAdGraphicsImageRendererFormat : NSObject
...
@property (nonatomic) TXAdGraphicsImageRendererFormatRange preferredRange;
...
@interface TXAdGraphicsImageRenderer : NSObject
...
/// @param size The size of images output from the renderer, specified in points.
/// @param format A TXAdGraphicsImageRendererFormat object that encapsulates the format used to create the renderer context.
/// @return An initialized image renderer.
- (nonnull instancetype)initWithSize:(CGSize)size format:(nonnull TXAdGraphicsImageRendererFormat *)format;

/// Creates an image by following a set of drawing instructions.
/// @param actions A TXAdGraphicsImageDrawingActions block that, when invoked by the renderer, executes a set of drawing instructions to create the output image.
...
- (nonnull UIImage *)imageWithActions:(nonnull NS_NOESCAPE TXAdGraphicsImageDrawingActions)actions;
