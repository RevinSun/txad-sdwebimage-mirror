...
/**
 This is the protocol for TXAdAnimatedImage class only but not for TXAdAnimatedImageCoder. If you want to provide a custom animated image class with full advanced function, you can conform to this instead of the base protocol.
 */
@protocol TXAdAnimatedImage <TXAdAnimatedImageProvider>
 ...
 */
- (nullable instancetype)initWithData:(nonnull NSData *)data scale:(CGFloat)scale options:(nullable TXAdImageCoderOptions *)options;

/**
 Initializes the image with an animated coder. You can use the coder to decode the image frame later.
 @note We use this with animated coder which conforms to `TXAdProgressiveImageCoder` for progressive animation decoding.
 
 @param animatedCoder An animated coder which conform `TXAdAnimatedImageCoder` protocol
 @param scale The scale factor to assume when interpreting the image data. Applying a scale factor of 1.0 results in an image whose size matches the pixel-based dimensions of the image. Applying a different scale factor changes the size of the image as reported by the `size` property.
 @return An initialized object
 */
- (nullable instancetype)initWithAnimatedCoder:(nonnull id<TXAdAnimatedImageCoder>)animatedCoder scale:(CGFloat)scale;
...
/**
 ...
 @note We use this with animated coder which conforms to `TXAdProgressiveImageCoder` for progressive animation decoding.
 */
@property (nonatomic, strong, readonly, nullable) id<TXAdAnimatedImageCoder> animatedCoder;

@end

/**
 The image class which supports animating on `TXAdAnimatedImageView`. You can also use it on normal UIImageView/NSImageView.
 */
NS_SWIFT_NONISOLATED
@interface TXAdAnimatedImage : UIImage <TXAdAnimatedImage>
...
// @note Before 5.19, these initializer will return nil for static image (when all candidate TXAdAnimatedImageCoder returns nil instance), like JPEG data. After 5.19, these initializer will retry for static image as well, so JPEG data will return non-nil instance. For vector image(PDF/SVG), always return nil.
// @note When the animated image frame count <= 1, all the `TXAdAnimatedImageProvider` protocol methods will return nil or 0 value, you'd better check the frame count before usage and keep fallback.
...
/**
 ...
 @note This actually just call `[NSData txad_imageFormatForImageData:self.animatedImageData]`
 */
...
// By default, animated image frames are returned by decoding just in time without keeping into memory. But you can choose to preload them into memory as well, See the description in `TXAdAnimatedImage` protocol.
