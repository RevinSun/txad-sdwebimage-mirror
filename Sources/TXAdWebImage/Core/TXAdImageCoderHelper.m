...
#import "TXAdInternalMacros.h"
#import "TXAdDeviceHelper.h"
#import <Accelerate/Accelerate.h>

#define kCGColorSpaceDeviceRGB CFSTR("kCGColorSpaceDeviceRGB")

#if TXAd_UIKIT
static inline UIImage *TXAdImageDecodeUIKit(UIImage *image) {
    ...
    if (@available(iOS 15, tvOS 15, *)) {
        ...
        if (decodedImage) {
            TXAdImageCopyAssociatedObject(image, decodedImage);
            decodedImage.txad_isDecoded = YES;
            return decodedImage;
        }
    }
    return nil;
...
static inline UIImage *TXAdImageDecodeAndScaleDownUIKit(UIImage *image, CGSize destResolution) {
...
static inline BOOL TXAdImageSupportsHardwareHEVCDecoder(void) {
    static dispatch_once_t onceToken;
    static BOOL supportsHardware = NO;
    dispatch_once(&onceToken, ^{
        SEL DeviceInfoSelector = TXAd_SEL_SPI(deviceInfoForKey:);
...
static UIImage * _Nonnull TXAdImageGetAlphaDummyImage(void) {
    static dispatch_once_t onceToken;
    static UIImage *dummyImage;
    dispatch_once(&onceToken, ^{
        TXAdGraphicsImageRendererFormat *format = [TXAdGraphicsImageRendererFormat preferredFormat];
        format.scale = 1;
        format.opaque = NO;
        CGSize size = CGSizeMake(1, 1);
        TXAdGraphicsImageRenderer *renderer = [[TXAdGraphicsImageRenderer alloc] initWithSize:size format:format];
...
static UIImage * _Nonnull TXAdImageGetNonAlphaDummyImage(void) {
    static dispatch_once_t onceToken;
    static UIImage *dummyImage;
    dispatch_once(&onceToken, ^{
        TXAdGraphicsImageRendererFormat *format = [TXAdGraphicsImageRendererFormat preferredFormat];
...
static TXAdImageCoderDecodeSolution kDefaultDecodeSolution = TXAdImageCoderDecodeSolutionAutomatic;
...
/*
 * Defines the maximum size in MB of the decoded image when the flag `TXAdWebImageScaleDownLargeImages` is set
...
#if TXAd_MAC
@interface TXAdAnimatedImageRep (Private)
...
@implementation TXAdImageCoderHelper

+ (UIImage *)animatedImageWithFrames:(NSArray<TXAdImageFrame *> *)frames {
    ...
    [frames enumerateObjectsUsingBlock:^(TXAdImageFrame * _Nonnull frame, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    CFStringRef imageUTType = [NSData txad_UTTypeFromImageFormat:TXAdImageFormatGIF];
    ...
    TXAdAnimatedImageRep *imageRep = [[TXAdAnimatedImageRep alloc] initWithData:imageData];
...
+ (NSArray<TXAdImageFrame *> *)framesFromAnimatedImage:(UIImage *)animatedImage {
    if (!animatedImage) {
        return nil;
    }
    
    NSMutableArray<TXAdImageFrame *> *frames;
    ...
    [animatedImages enumerateObjectsUsingBlock:^(UIImage * _Nonnull image, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([image isEqual:previousImage]) {
            ...
            TXAdImageFrame *frame = [TXAdImageFrame frameWithImage:previousImage duration:avgDuration * repeatCount];
    ...
    for (size_t i = 0; i < frameCount; i++) {
        ...
        TXAdImageFrame *frame = [TXAdImageFrame frameWithImage:frameImage duration:frameDuration];
        [frames addObject:frame];
    }
#endif
...
+ (TXAdImagePixelFormat)preferredPixelFormat:(BOOL)containsAlpha {
    CGImageRef cgImage;
    if (containsAlpha) {
        cgImage = TXAdImageGetAlphaDummyImage().CGImage;
    } else {
        cgImage = TXAdImageGetNonAlphaDummyImage().CGImage;
    }
    ...
    TXAdImagePixelFormat pixelFormat = {
...
+ (BOOL)CGImageIsHardwareSupported:(CGImageRef)cgImage {
    ...
    TXAdImagePixelFormat pixelFormat = [self preferredPixelFormat:hasAlpha];
    if (TXAdByteAlign(bytesPerRow, pixelFormat.alignment) == bytesPerRow) {
        // byte aligned, OK
        supported &= YES;
    } else {
        // not aligned
...
+ (CGImageRef)CGImageCreateDecoded:(CGImageRef)cgImage orientation:(CGImagePropertyOrientation)orientation {
    ...
    CGBitmapInfo bitmapInfo = [TXAdImageCoderHelper preferredPixelFormat:hasAlpha].bitmapInfo;
    ...
    // Apply transform
    CGAffineTransform transform = TXAdCGContextTransformFromOrientation(orientation, CGSizeMake(newWidth, newHeight));
...
+ (UIImage *)decodedImageWithImage:(UIImage *)image {
    return [self decodedImageWithImage:image policy:TXAdImageForceDecodePolicyAutomatic];
}

+ (UIImage *)decodedImageWithImage:(UIImage *)image policy:(TXAdImageForceDecodePolicy)policy {
    if (![self shouldDecodeImage:image policy:policy]) {
        return image;
    }
    
    UIImage *decodedImage;
    TXAdImageCoderDecodeSolution decodeSolution = self.defaultDecodeSolution;
#if TXAd_UIKIT
    if (decodeSolution == TXAdImageCoderDecodeSolutionAutomatic) {
        ...
        if ((format == TXAdImageFormatHEIC || format == TXAdImageFormatHEIF) && TXAdImageSupportsHardwareHEVCDecoder()) {
            decodedImage = TXAdImageDecodeUIKit(image);
        } else if (format == TXAdImageFormatJPEG) {
            decodedImage = TXAdImageDecodeUIKit(image);
        }
    } else if (decodeSolution == TXAdImageCoderDecodeSolutionUIKit) {
    ...
    if (decodeSolution == TXAdImageCoderDecodeSolutionCoreGraphics) {
        ...
        TXAdGraphicsImageRenderer *renderer = [[TXAdGraphicsImageRenderer alloc] initWithSize:imageSize format:format];
...
+ (UIImage *)decodedAndScaledDownImageWithImage:(UIImage *)image limitBytes:(NSUInteger)bytes {
    return [self decodedAndScaledDownImageWithImage:image limitBytes:bytes policy:TXAdImageForceDecodePolicyAutomatic];
}

+ (UIImage *)decodedAndScaledDownImageWithImage:(UIImage *)image limitBytes:(NSUInteger)bytes policy:(TXAdImageForceDecodePolicy)policy {
    if (![self shouldDecodeImage:image policy:policy]) {
        return image;
    }
    ...
    if (decodeSolution == TXAdImageCoderDecodeSolutionAutomatic) {
        ...
        TXAdImageFormat format = image.txad_imageFormat;
        if ((format == TXAdImageFormatHEIC || format == TXAdImageFormatHEIF) && TXAdImageSupportsHardwareHEVCDecoder()) {
            decodedImage = TXAdImageDecodeAndScaleDownUIKit(image, destResolution);
        } else if (format == TXAdImageFormatJPEG) {
            decodedImage = TXAdImageDecodeAndScaleDownUIKit(image, destResolution);
        }
    } else if (decodeSolution == TXAdImageCoderDecodeSolutionUIKit) {
    ...
    // on iOS7, do not forget to call [[TXAdImageCache sharedImageCache] clearMemory];
    @autoreleasepool {
        // device color space
...
+ (TXAdImageCoderDecodeSolution)defaultDecodeSolution {
    return kDefaultDecodeSolution;
}

+ (void)setDefaultDecodeSolution:(TXAdImageCoderDecodeSolution)defaultDecodeSolution {
...
+ (BOOL)shouldDecodeImage:(nullable UIImage *)image policy:(TXAdImageForceDecodePolicy)policy {
    // Prevent "CGBitmapContextCreateImage: invalid context 0x0" error
    if (image == nil) {
        return NO;
    }
    // Check policy (never)
    if (policy == TXAdImageForceDecodePolicyNever) {
        return NO;
    }
    // Avoid extra decode
    if (image.txad_isDecoded) {
        return NO;
    }
    ...
    // Check policy (always)
    if (policy == TXAdImageForceDecodePolicyAlways) {
        return YES;
    } else {
        ...
        if (cgImage) {
            ...
            BOOL isLazy = [TXAdImageCoderHelper CGImageIsLazy:cgImage];
            if (isLazy) {
                ...
                BOOL isSupported = [TXAdImageCoderHelper CGImageIsHardwareSupported:cgImage];
                return !isSupported;
            }
        }
    }
...
static inline CGAffineTransform TXAdCGContextTransformFromOrientation(CGImagePropertyOrientation orientation, CGSize size) {
