...
@interface TXAdWebImagePrefetchToken () {
    @public
    // Though current implementation, `TXAdWebImageManager` completion block is always on main queue. But however, there is no guarantee in docs. And we may introduce config to specify custom queue in the future.
    ...
    TXAd_LOCK_DECLARE(_prefetchOperationsLock);
    TXAd_LOCK_DECLARE(_loadOperationsLock);
}
...
@property (nonatomic, weak) TXAdWebImagePrefetcher *prefetcher;
@property (nonatomic, assign) TXAdWebImageOptions options;
@property (nonatomic, copy, nullable) TXAdWebImageContext *context;
@property (nonatomic, copy, nullable) TXAdWebImagePrefetcherCompletionBlock completionBlock;
@property (nonatomic, copy, nullable) TXAdWebImagePrefetcherProgressBlock progressBlock;

@end

@interface TXAdWebImagePrefetcher ()

@property (strong, nonatomic, nonnull) TXAdWebImageManager *manager;
@property (strong, atomic, nonnull) NSMutableSet<TXAdWebImagePrefetchToken *> *runningTokens;
@property (strong, nonatomic, nonnull) NSOperationQueue *prefetchQueue;
@property (strong, nonatomic, nullable) TXAdCallbackQueue *callbackQueue;

@end

@implementation TXAdWebImagePrefetcher
...
- (nonnull instancetype)init {
    return [self initWithImageManager:[TXAdWebImageManager new]];
}

- (nonnull instancetype)initWithImageManager:(TXAdWebImageManager *)manager {
    if ((self = [super init])) {
        _manager = manager;
        _runningTokens = [NSMutableSet set];
        _options = TXAdWebImageLowPriority;
...
- (void)setDelegateQueue:(dispatch_queue_t)delegateQueue {
    // Deprecate and translate to TXAdCallbackQueue
    _callbackQueue = [[TXAdCallbackQueue alloc] initWithDispatchQueue:delegateQueue];
    _callbackQueue.policy = TXAdCallbackPolicyDispatch;
}

- (dispatch_queue_t)delegateQueue {
    // Deprecate and translate to TXAdCallbackQueue
    return (_callbackQueue ?: TXAdCallbackQueue.mainQueue).queue;
}
...
- (nullable TXAdWebImagePrefetchToken *)prefetchURLs:(nullable NSArray<NSURL *> *)urls
    ...
    TXAdWebImagePrefetchToken *token = [TXAdWebImagePrefetchToken new];
...
- (void)startPrefetchWithToken:(TXAdWebImagePrefetchToken * _Nonnull)token {
    for (NSURL *url in token.urls) {
        @weakify(self);
        TXAdAsyncBlockOperation *prefetchOperation = [TXAdAsyncBlockOperation blockOperationWithBlock:^(TXAdAsyncBlockOperation * _Nonnull asyncOperation) {
            @strongify(self);
            if (!self || asyncOperation.isCancelled) {
                return;
            }
            id<TXAdWebImageOperation> operation = [self.manager loadImageWithURL:url options:token.options context:token.context progress:nil completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, TXAdImageCacheType cacheType, BOOL finished, NSURL * _Nullable imageURL) {
                @strongify(self);
                if (!self) {
                    return;
                }
            ...
            NSAssert(operation != nil, @"Operation should not be nil, [TXAdWebImageManager loadImageWithURL:options:context:progress:completed:] break prefetch logic");
            TXAd_LOCK(token->_loadOperationsLock);
            [token.loadOperations addPointer:(__bridge void *)operation];
            TXAd_UNLOCK(token->_loadOperationsLock);
        }];
        TXAd_LOCK(token->_prefetchOperationsLock);
        [token.prefetchOperations addPointer:(__bridge void *)prefetchOperation];
        TXAd_UNLOCK(token->_prefetchOperationsLock);
...
- (void)cancelPrefetching {
    @synchronized(self.runningTokens) {
        NSSet<TXAdWebImagePrefetchToken *> *copiedTokens = [self.runningTokens copy];
        [copiedTokens makeObjectsPerformSelector:@selector(cancel)];
        [self.runningTokens removeAllObjects];
    }
}

- (void)callProgressBlockForToken:(TXAdWebImagePrefetchToken *)token imageURL:(NSURL *)url {
    if (!token) {
        return;
    }
    ...
    TXAdCallbackQueue *queue = token.context[TXAdWebImageContextCallbackQueue];
    if (!queue) {
...
- (void)callCompletionBlockForToken:(TXAdWebImagePrefetchToken *)token {
    if (!token) {
        return;
    }
...
- (NSUInteger)tokenTotalCount {
    ...
    @synchronized (self.runningTokens) {
        for (TXAdWebImagePrefetchToken *token in self.runningTokens) {
...
- (void)addRunningToken:(TXAdWebImagePrefetchToken *)token {
    if (!token) {
        return;
    }
    @synchronized (self.runningTokens) {
...
- (void)removeRunningToken:(TXAdWebImagePrefetchToken *)token {
    if (!token) {
        return;
    }
    @synchronized (self.runningTokens) {
...
@implementation TXAdWebImagePrefetchToken

- (instancetype)init {
    self = [super init];
    if (self) {
        TXAd_LOCK_INIT(_prefetchOperationsLock);
        TXAd_LOCK_INIT(_loadOperationsLock);
    }
    return self;
}

- (void)cancel {
    TXAd_LOCK(_prefetchOperationsLock);
    [self.prefetchOperations compact];
    for (id operation in self.prefetchOperations) {
        id<TXAdWebImageOperation> strongOperation = operation;
        if (strongOperation) {
            [strongOperation cancel];
        }
    }
    self.prefetchOperations.count = 0;
    TXAd_UNLOCK(_prefetchOperationsLock);
    
    TXAd_LOCK(_loadOperationsLock);
    ...
    TXAd_UNLOCK(_loadOperationsLock);
