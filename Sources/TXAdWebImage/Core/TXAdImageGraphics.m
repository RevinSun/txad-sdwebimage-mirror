...
static CGContextRef TXAdCGContextCreateBitmapContext(CGSize size, BOOL opaque, CGFloat scale) {
    if (scale == 0) {
    ...
    CGColorSpaceRef space = [TXAdImageCoderHelper colorSpaceGetDeviceRGB];
...
CGContextRef TXAdGraphicsGetCurrentContext(void) {
...
void TXAdGraphicsBeginImageContext(CGSize size) {
#if TXAd_UIKIT || TXAd_WATCH
    UIGraphicsBeginImageContext(size);
#else
    TXAdGraphicsBeginImageContextWithOptions(size, NO, 1.0);
#endif
}

void TXAdGraphicsBeginImageContextWithOptions(CGSize size, BOOL opaque, CGFloat scale) {
#if TXAd_UIKIT || TXAd_WATCH
    UIGraphicsBeginImageContextWithOptions(size, opaque, scale);
#else
    CGContextRef context = TXAdCGContextCreateBitmapContext(size, opaque, scale);
    if (!context) {
        return;
    }
...
void TXAdGraphicsEndImageContext(void) {
#if TXAd_UIKIT || TXAd_WATCH
...
UIImage * TXAdGraphicsGetImageFromCurrentImageContext(void) {
#if TXAd_UIKIT || TXAd_WATCH
