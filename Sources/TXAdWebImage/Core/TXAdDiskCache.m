...
#import "TXAdFileAttributeHelper.h"
#import <CommonCrypto/CommonDigest.h>

static NSString * const TXAdDiskCacheExtendedAttributeName = @"com.hackemist.TXAdDiskCache";

@interface TXAdDiskCache ()
...
- (instancetype)initWithCachePath:(NSString *)cachePath config:(nonnull TXAdImageCacheConfig *)config {
    if (self = [super init]) {
...
- (BOOL)containsDataForKey:(NSString *)key {
    ...
    // fallback because of https://github.com/rs/TXAdWebImage/pull/976 that added the extension to the disk file name
...
- (NSData *)extendedDataForKey:(NSString *)key {
    ...
    NSData *extendedData = [TXAdFileAttributeHelper extendedAttribute:TXAdDiskCacheExtendedAttributeName atPath:cachePathForKey traverseLink:NO error:nil];
...
- (void)setExtendedData:(NSData *)extendedData forKey:(NSString *)key {
    ...
    if (!extendedData) {
        // Remove
        [TXAdFileAttributeHelper removeExtendedAttribute:TXAdDiskCacheExtendedAttributeName atPath:cachePathForKey traverseLink:NO error:nil];
    } else {
        // Override
        [TXAdFileAttributeHelper setExtendedAttribute:TXAdDiskCacheExtendedAttributeName value:extendedData atPath:cachePathForKey traverseLink:NO overwrite:YES error:nil];
    }
}
...
- (void)removeExpiredData {
    ...
    switch (self.config.diskCacheExpireType) {
        case TXAdImageCacheConfigExpireTypeAccessDate:
            cacheContentDateKey = NSURLContentAccessDateKey;
            break;
        case TXAdImageCacheConfigExpireTypeModificationDate:
            cacheContentDateKey = NSURLContentModificationDateKey;
            break;
        case TXAdImageCacheConfigExpireTypeCreationDate:
            cacheContentDateKey = NSURLCreationDateKey;
            break;
        case TXAdImageCacheConfigExpireTypeChangeDate:
...
- (nullable NSString *)cachePathForKey:(nullable NSString *)key inPath:(nonnull NSString *)path {
    NSString *filename = TXAdDiskCacheFileNameForKey(key);
...
static inline NSString *TXAdSanitizeFileNameString(NSString * _Nullable fileName) {
...
#define TXAd_MAX_FILE_EXTENSION_LENGTH (NAME_MAX - CC_MD5_DIGEST_LENGTH * 2 - 1)

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
static inline NSString * _Nonnull TXAdDiskCacheFileNameForKey(NSString * _Nullable key) {
    ...
    ext = TXAdSanitizeFileNameString(ext);
    // File system has file name length limit, we need to check if ext is too long, we don't add it to the filename
    if (ext.length > TXAd_MAX_FILE_EXTENSION_LENGTH) {
        ext = nil;
    }
