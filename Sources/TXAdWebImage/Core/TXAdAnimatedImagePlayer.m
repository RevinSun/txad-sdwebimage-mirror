...
#import "TXAdImageFramePool.h"
#import "TXAdInternalMacros.h"

@interface TXAdAnimatedImagePlayer () {
    NSRunLoopMode _runLoopMode;
}

@property (nonatomic, strong) TXAdImageFramePool *framePool;

@property (nonatomic, strong, readwrite) UIImage *currentFrame;
@property (nonatomic, assign, readwrite) NSUInteger currentFrameIndex;
@property (nonatomic, assign, readwrite) NSUInteger currentLoopCount;
@property (nonatomic, strong) id<TXAdAnimatedImageProvider> animatedProvider;
...
@property (nonatomic, strong) TXAdDisplayLink *displayLink;

@end

@implementation TXAdAnimatedImagePlayer

- (instancetype)initWithProvider:(id<TXAdAnimatedImageProvider>)provider {
    self = [super init];
    if (self) {
        ...
        self.framePool = [TXAdImageFramePool registerProvider:provider];
    }
    return self;
}

+ (instancetype)playerWithProvider:(id<TXAdAnimatedImageProvider>)provider {
    TXAdAnimatedImagePlayer *player = [[TXAdAnimatedImagePlayer alloc] initWithProvider:provider];
    return player;
}

- (void)dealloc {
    // Dereference the frame pool, when zero the frame pool for provider will dealloc
    [TXAdImageFramePool unregisterProvider:self.animatedProvider];
}

#pragma mark - Private

- (TXAdDisplayLink *)displayLink {
    if (!_displayLink) {
        _displayLink = [TXAdDisplayLink displayLinkWithTarget:self selector:@selector(displayDidRefresh:)];
...
- (void)setupCurrentFrame {
    ...
    if (self.playbackMode == TXAdAnimatedImagePlaybackModeReverse ||
               self.playbackMode == TXAdAnimatedImagePlaybackModeReversedBounce) {
...
- (void)stopPlaying {
    // Using `_displayLink` here because when UIImageView dealloc, it may trigger `[self stopAnimating]`, we already release the display link in TXAdAnimatedImageView's dealloc method.
    [_displayLink stop];
...
- (void)displayDidRefresh:(TXAdDisplayLink *)displayLink {
    ...
    } else if (self.playbackMode == TXAdAnimatedImagePlaybackModeBounce ||
               self.playbackMode == TXAdAnimatedImagePlaybackModeReversedBounce) {
...
- (void)calculateMaxBufferCountWithFrame:(nonnull UIImage *)frame {
    ...
    if (self.maxBufferSize > 0) {
        ...
        NSUInteger total = [TXAdDeviceHelper totalMemory];
        NSUInteger free = [TXAdDeviceHelper freeMemory];
