...
@interface TXAdAnimatedImageView () {
    BOOL _initFinished; // Extra flag to mark the `commonInit` is called
    NSRunLoopMode _runLoopMode;
    NSUInteger _maxBufferSize;
    double _playbackRate;
    TXAdAnimatedImagePlaybackMode _playbackMode;
}

@property (nonatomic, strong, readwrite) TXAdAnimatedImagePlayer *player;
...
@implementation TXAdAnimatedImageView
#if TXAd_UIKIT
...
+ (instancetype)imageViewWithImage:(NSImage *)image
{
    ...
    TXAdAnimatedImageView *imageView = [[TXAdAnimatedImageView alloc] initWithFrame:frame];
...
- (void)setImage:(UIImage *)image
{
    ...
    super.image = image;
    if ([image.class conformsToProtocol:@protocol(TXAdAnimatedImage)] && [(id<TXAdAnimatedImage>)image animatedImageFrameCount] > 1) {
        if (!self.player) {
            id<TXAdAnimatedImageProvider> provider;
            // Check progressive loading
            if (self.isProgressive) {
                provider = [(id<TXAdAnimatedImage>)image animatedCoder];
            } else {
                provider = (id<TXAdAnimatedImage>)image;
            }
            // Create animated player
            self.player = [TXAdAnimatedImagePlayer playerWithProvider:provider];
        } else {
            // Update Frame Count
            self.player.totalFrameCount = [(id<TXAdAnimatedImage>)image animatedImageFrameCount];
        }
...
- (void)setPlaybackMode:(TXAdAnimatedImagePlaybackMode)playbackMode {
    _playbackMode = playbackMode;
    self.player.playbackMode = playbackMode;
}

- (TXAdAnimatedImagePlaybackMode)playbackMode {
    if (!_initFinished) {
        return TXAdAnimatedImagePlaybackModeNormal; // Default mode is normal
    }
...
- (void)checkPlay
{
    // Only handle for TXAdAnimatedImage, leave UIAnimatedImage or animationImages for super implementation control
...
- (void)updateIsProgressiveWithImage:(UIImage *)image
{
    ...
    id<TXAdAnimatedImageCoder> currentAnimatedCoder = [self progressiveAnimatedCoderForImage:image];
    if (currentAnimatedCoder) {
        ...
        if (!previousImage) {
            ...
            id<TXAdAnimatedImageCoder> previousAnimatedCoder = [self progressiveAnimatedCoderForImage:previousImage];
...
- (id<TXAdAnimatedImageCoder, TXAdProgressiveImageCoder>)progressiveAnimatedCoderForImage:(UIImage *)image
{
    if ([image.class conformsToProtocol:@protocol(TXAdAnimatedImage)] && image.txad_isIncremental && [image respondsToSelector:@selector(animatedCoder)]) {
        id<TXAdAnimatedImageCoder> animatedCoder = [(id<TXAdAnimatedImage>)image animatedCoder];
        if ([animatedCoder respondsToSelector:@selector(initIncrementalWithOptions:)]) {
            return (id<TXAdAnimatedImageCoder, TXAdProgressiveImageCoder>)animatedCoder;
        }
    }
    return nil;
}
...
- (NSView *)imageView {
    NSImageView *imageView = objc_getAssociatedObject(self, TXAd_SEL_SPI(imageView));
    if (!imageView) {
        // macOS 10.14
        imageView = objc_getAssociatedObject(self, TXAd_SEL_SPI(imageSubview));
    }
    return imageView;
}
