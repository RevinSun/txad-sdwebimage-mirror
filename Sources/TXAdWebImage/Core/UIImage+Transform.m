...
static inline CGRect TXAdCGRectFitWithScaleMode(CGRect rect, CGSize size, TXAdImageScaleMode scaleMode) {
    rect = CGRectStandardize(rect);
    size.width = size.width < 0 ? -size.width : size.width;
    size.height = size.height < 0 ? -size.height : size.height;
    CGPoint center = CGPointMake(CGRectGetMidX(rect), CGRectGetMidY(rect));
    switch (scaleMode) {
        case TXAdImageScaleModeAspectFit:
        case TXAdImageScaleModeAspectFill: {
            if (rect.size.width < 0.01 || rect.size.height < 0.01 ||
                ...
                CGFloat scale;
                if (scaleMode == TXAdImageScaleModeAspectFit) {
...
static inline UIColor * TXAdGetColorFromGrayscale(Pixel_88 pixel, CGBitmapInfo bitmapInfo, CGColorSpaceRef cgColorSpace) {
...
static inline UIColor * TXAdGetColorFromRGBA(Pixel_8888 pixel, CGBitmapInfo bitmapInfo, CGColorSpaceRef cgColorSpace) {
    // Get alpha info, byteOrder info
    CGImageAlphaInfo alphaInfo = bitmapInfo & kCGBitmapAlphaInfoMask;
    CGBitmapInfo byteOrderInfo = bitmapInfo & kCGBitmapByteOrderMask;
...
static inline CGImageRef _Nullable TXAdCreateCGImageFromCIImage(CIImage * _Nonnull ciImage) {
...
- (void)txad_drawInRect:(CGRect)rect context:(CGContextRef)context scaleMode:(TXAdImageScaleMode)scaleMode clipsToBounds:(BOOL)clips {
    CGRect drawRect = TXAdCGRectFitWithScaleMode(rect, self.size, scaleMode);
...
- (nullable UIImage *)txad_resizedImageWithSize:(CGSize)size scaleMode:(TXAdImageScaleMode)scaleMode {
    ...
    TXAdGraphicsImageRendererFormat *format = [[TXAdGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
    TXAdGraphicsImageRenderer *renderer = [[TXAdGraphicsImageRenderer alloc] initWithSize:size format:format];
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
        [self txad_drawInRect:CGRectMake(0, 0, size.width, size.height) context:context scaleMode:scaleMode clipsToBounds:NO];
    }];
    return image;
}
...
- (nullable UIImage *)txad_roundedCornerImageWithRadius:(CGFloat)cornerRadius corners:(TXAdRectCorner)corners borderWidth:(CGFloat)borderWidth borderColor:(nullable UIColor *)borderColor {
    TXAdGraphicsImageRendererFormat *format = [[TXAdGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
    TXAdGraphicsImageRenderer *renderer = [[TXAdGraphicsImageRenderer alloc] initWithSize:self.size format:format];
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
...
#else
            NSBezierPath *path = [NSBezierPath txad_bezierPathWithRoundedRect:CGRectInset(rect, borderWidth, borderWidth) byRoundingCorners:corners cornerRadius:cornerRadius];
#endif
            [path closePath];
...
#else
            NSBezierPath *path = [NSBezierPath txad_bezierPathWithRoundedRect:strokeRect byRoundingCorners:corners cornerRadius:strokeRadius];
#endif
            [path closePath];
...
- (nullable UIImage *)txad_rotatedImageWithAngle:(CGFloat)angle fitSize:(BOOL)fitSize {
    ...
    TXAdGraphicsImageRendererFormat *format = [[TXAdGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
    TXAdGraphicsImageRenderer *renderer = [[TXAdGraphicsImageRenderer alloc] initWithSize:newRect.size format:format];
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
...
- (nullable UIColor *)txad_colorAtPoint:(CGPoint)point {
...
#if TXAd_UIKIT || TXAd_MAC
    if (self.CIImage) {
        imageRef = TXAdCreateCGImageFromCIImage(self.CIImage);
    }
#endif
    if (!imageRef) {
    ...
    if (components == 2) {
        ...
        CGImageRelease(imageRef);
        // Convert to color
        return TXAdGetColorFromGrayscale(pixel, bitmapInfo, colorSpace);
        ...
        CGImageRelease(imageRef);
        // Convert to color
        return TXAdGetColorFromRGBA(pixel, bitmapInfo, colorSpace);
    } else {
        TXAd_LOG("Unsupported components: %zu", components);
        CFRelease(data);
        CGImageRelease(imageRef);
        return nil;
    }
...
- (nullable NSArray<UIColor *> *)txad_colorsWithRect:(CGRect)rect {
    ...
    for (size_t index = start; index < end; index += components) {
        ...
        if (components == 2) {
            ...
            color = TXAdGetColorFromGrayscale(pixel, bitmapInfo, colorSpace);
        } else {
            if (components == 3) {
                Pixel_8888 pixel = {pixels[index], pixels[index+1], pixels[index+2], 0};
                color = TXAdGetColorFromRGBA(pixel, bitmapInfo, colorSpace);
...
- (nullable UIImage *)txad_blurredImageWithRadius:(CGFloat)blurRadius {
    ...
    if (err != kvImageNoError) {
        TXAd_LOG("UIImage+Transform error: vImageBuffer_InitWithCGImage returned error code %zi for inputImage: %@", err, self);
        return nil;
    }
    err = vImageBuffer_Init(&scratch, effect.height, effect.width, format.bitsPerPixel, kvImageNoFlags);
    if (err != kvImageNoError) {
        TXAd_LOG("UIImage+Transform error: vImageBuffer_Init returned error code %zi for inputImage: %@", err, self);
        return nil;
    }
