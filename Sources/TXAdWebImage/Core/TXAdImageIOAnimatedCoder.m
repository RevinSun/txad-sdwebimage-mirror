...
#if TXAd_CHECK_CGIMAGE_RETAIN_SOURCE
#import <dlfcn.h>

// SPI to check thread safe during Example and Test
static CGImageSourceRef (*TXAdCGImageGetImageSource)(CGImageRef);
#endif

// Specify File Size for lossy format encoding, like JPEG
static NSString * kTXAdCGImageDestinationRequestedFileSize = @"kCGImageDestinationRequestedFileSize";
...
static CGImageRef __nullable TXAdCGImageCreateMutableCopy(CGImageRef cg_nullable image, CGBitmapInfo bitmapInfo) {
    if (!image) return nil;
...
static inline BOOL TXAdCGImageIs8Bit(CGImageRef cg_nullable image) {
    return CGImageGetBitsPerComponent(image) == 8;
}

static inline CGImageRef __nullable TXAdCGImageCreateCopy(CGImageRef cg_nullable image) {
    if (!image) return nil;
    return TXAdCGImageCreateMutableCopy(image, CGImageGetBitmapInfo(image));
}

static BOOL TXAdLoadOnePixelBitmapBuffer(CGImageRef imageRef, uint8_t *r, uint8_t *g, uint8_t *b, uint8_t *a) {
...
static CGImageRef TXAdImageIOPNGPluginBuggyCreateWorkaround(CGImageRef cgImage) CF_RETURNS_RETAINED {
    ...
    if (newAlphaInfo != alphaInfo) {
        ...
        if (TXAd_OPTIONS_CONTAINS(bitmapInfo, kCGBitmapFloatComponents)) {
            // Keep float components
            newBitmapInfo |= kCGBitmapFloatComponents;
        }
        // Create new CGImage with corrected alpha info...
        CGImageRef newCGImage = TXAdCGImageCreateMutableCopy(cgImage, newBitmapInfo);
        return newCGImage;
    } else {
...
static BOOL TXAdImageIOPNGPluginBuggyNeedWorkaround(void) {
    // See: #3605 FB13322459
    ...
    dispatch_once(&onceToken, ^{
        ...
        uint8_t r, g, b, a;
        r = g = b = a = 0;
        BOOL success = TXAdLoadOnePixelBitmapBuffer(cgImage, &r, &g, &b, &a);
        if (!success) {
            ...
            if (r == 50 && g == 50 && b == 50 && a == 50) {
                ...
                TXAd_LOG("%@", @"Detected the current OS's ImageIO PNG Decoder is buggy on indexed color PNG. Perform workaround solution...");
                isBuggy = YES;
            }
        }
        CFRelease(source);
...
@interface TXAdImageIOCoderFrame : NSObject
...
@implementation TXAdImageIOCoderFrame
@end

@implementation TXAdImageIOAnimatedCoder {
    size_t _width, _height;
    CGImageSourceRef _imageSource;
    BOOL _incremental;
    TXAd_LOCK_DECLARE(_lock); // Lock only apply for incremental animation decoding
    NSData *_imageData;
    CGFloat _scale;
    NSUInteger _loopCount;
    NSUInteger _frameCount;
    NSArray<TXAdImageIOCoderFrame *> *_frames;
    BOOL _finished;
...
+ (TXAdImageFormat)imageFormat {
    @throw [NSException exceptionWithName:NSInternalInconsistencyException
                                   reason:[NSString stringWithFormat:@"For `TXAdImageIOAnimatedCoder` subclass, you must override %@ method", NSStringFromSelector(_cmd)]
                                 userInfo:nil];
}
...
+ (BOOL)canDecodeFromFormat:(TXAdImageFormat)format {
    ...
    CFStringRef imageUTType = [NSData txad_UTTypeFromImageFormat:format];
...
+ (UIImage *)createFrameAtIndex:(NSUInteger)index source:(CGImageSourceRef)source scale:(CGFloat)scale preserveAspectRatio:(BOOL)preserveAspectRatio thumbnailSize:(CGSize)thumbnailSize lazyDecode:(BOOL)lazyDecode animatedImage:(BOOL)animatedImage {
    // `animatedImage` means called from `TXAdAnimatedImageProvider.animatedImageFrameAtIndex`
    ...
    if (!createFullImage) {
        if (preserveAspectRatio) {
            ...
            CGImageRef scaledImageRef = [TXAdImageCoderHelper CGImageCreateScaled:imageRef size:thumbnailSize];
            if (scaledImageRef) {
    ...
    BOOL isLazy = [TXAdImageCoderHelper CGImageIsLazy:imageRef];
    if (!lazyDecode) {
        if (isLazy) {
            // Use CoreGraphics to trigger immediately decode to drop lazy CGImage
            CGImageRef decodedImageRef = [TXAdImageCoderHelper CGImageCreateDecoded:imageRef];
            if (decodedImageRef) {
                CGImageRelease(imageRef);
        ...
        // iOS 15+, CGImageRef now retains CGImageSourceRef internally. To workaround its thread-safe issue, we have to strip CGImageSourceRef, using Force-Decode (or have to use SPI `CGImageSetImageSource`), See: https://github.com/TXAdWebImage/TXAdWebImage/issues/3273
        if (@available(iOS 15, tvOS 15, *)) {
            ...
            CGImageRef newImageRef = TXAdCGImageCreateCopy(imageRef);
            if (newImageRef) {
                CGImageRelease(imageRef);
            ...
            // If assert hit, fire issue to https://github.com/TXAdWebImage/TXAdWebImage/issues and we update the condition for this behavior check
            static dispatch_once_t onceToken;
            dispatch_once(&onceToken, ^{
                TXAdCGImageGetImageSource = dlsym(RTLD_DEFAULT, "CGImageGetImageSource");
            });
            if (TXAdCGImageGetImageSource) {
                NSCAssert(!TXAdCGImageGetImageSource(imageRef), @"Animated Coder created CGImageRef should not retain CGImageSourceRef, which may cause thread-safe issue without lock");
            }
#endif
        }
    }
    ...
    TXAdImageFormat imageFormat = [NSData txad_imageFormatFromUTType:uttype];
    if (imageFormat == TXAdImageFormatPNG && TXAdCGImageIs8Bit(imageRef) && TXAdImageIOPNGPluginBuggyNeedWorkaround()) {
        CGImageRef newImageRef = TXAdImageIOPNGPluginBuggyCreateWorkaround(imageRef);
        CGImageRelease(imageRef);
        imageRef = newImageRef;
    }
    
#if TXAd_UIKIT || TXAd_WATCH
    UIImageOrientation imageOrientation = [TXAdImageCoderHelper imageOrientationFromEXIFOrientation:exifOrientation];
    UIImage *image = [[UIImage alloc] initWithCGImage:imageRef scale:scale orientation:imageOrientation];
#else
    UIImage *image = [[UIImage alloc] initWithCGImage:imageRef scale:scale orientation:exifOrientation];
#endif
    CGImageRelease(imageRef);
    image.txad_isDecoded = !isLazy;
...
- (BOOL)canDecodeFromData:(nullable NSData *)data {
    return ([NSData txad_imageFormatForImageData:data] == self.class.imageFormat);
}
...
- (UIImage *)decodedImageWithData:(NSData *)data options:(nullable TXAdImageCoderOptions *)options {
    ...
    CGFloat scale = 1;
    NSNumber *scaleFactor = options[TXAdImageCoderDecodeScaleFactor];
    if (scaleFactor != nil) {
    ...
    NSNumber *lazyDecodeValue = options[TXAdImageCoderDecodeUseLazyDecoding];
    if (lazyDecodeValue != nil) {
        lazyDecode = lazyDecodeValue.boolValue;
    }
    
    NSUInteger limitBytes = 0;
    NSNumber *limitBytesValue = options[TXAdImageCoderDecodeScaleDownLimitBytes];
    ...
    if (limitBytes == 0 && (thumbnailSize.width == 0 || thumbnailSize.height == 0)) {
        ...
        if (imageRep) {
            ...
            animatedImage.txad_imageFormat = self.class.imageFormat;
            return animatedImage;
        }
    }
#endif
    
    NSString *typeIdentifierHint = options[TXAdImageCoderDecodeTypeIdentifierHint];
    if (!typeIdentifierHint) {
        ...
        NSString *fileExtensionHint = options[TXAdImageCoderDecodeFileExtensionHint];
    ...
    if (limitBytes > 0) {
        ...
        CGSize framePixelSize = [TXAdImageCoderHelper scaledSizeWithImageSize:imageSize limitBytes:limitBytes bytesPerPixel:4 frameCount:frameCount];
        // Override thumbnail size
        thumbnailSize = framePixelSize;
        preserveAspectRatio = YES;
    }
    
    BOOL decodeFirstFrame = [options[TXAdImageCoderDecodeFirstFrameOnly] boolValue];
    if (decodeFirstFrame || frameCount <= 1) {
        ...
        animatedImage = [TXAdImageCoderHelper animatedImageWithFrames:frames];
        animatedImage.txad_imageLoopCount = loopCount;
    }
    animatedImage.txad_imageFormat = self.class.imageFormat;
    CFRelease(source);
...
- (instancetype)initIncrementalWithOptions:(nullable TXAdImageCoderOptions *)options {
    self = [super init];
    if (self) {
...
- (void)updateIncrementalData:(NSData *)data finished:(BOOL)finished {
    ...
    if (_limitBytes > 0) {
        ...
        CGSize framePixelSize = [TXAdImageCoderHelper scaledSizeWithImageSize:imageSize limitBytes:_limitBytes bytesPerPixel:4 frameCount:_frameCount];
        // Override thumbnail size
...
- (UIImage *)incrementalDecodedImageWithOptions:(TXAdImageCoderOptions *)options {
    NSCParameterAssert(_incremental);
    UIImage *image;
    ...
    if (_width + _height > 0) {
        ...
        if (image) {
            image.txad_imageFormat = self.class.imageFormat;
        }
    }
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(TXAdImageFormat)format options:(nullable TXAdImageCoderOptions *)options {
    ...
    NSArray<TXAdImageFrame *> *frames = [TXAdImageCoderHelper framesFromAnimatedImage:image];
    if (!frames || frames.count == 0) {
        TXAdImageFrame *frame = [TXAdImageFrame frameWithImage:image duration:0];
        frames = @[frame];
    }
    return [self encodedDataWithFrames:frames loopCount:image.txad_imageLoopCount format:format options:options];
}
...
- (NSData *)encodedDataWithFrames:(NSArray<TXAdImageFrame *> *)frames loopCount:(NSUInteger)loopCount format:(TXAdImageFormat)format options:(TXAdImageCoderOptions *)options {
...
#if TXAd_UIKIT || TXAd_WATCH
    CGImagePropertyOrientation exifOrientation = [TXAdImageCoderHelper exifOrientationFromImageOrientation:image.imageOrientation];
#else
    CGImagePropertyOrientation exifOrientation = kCGImagePropertyOrientationUp;
#endif
    ...
    if (options[TXAdImageCoderEncodeCompressionQuality]) {
        compressionQuality = [options[TXAdImageCoderEncodeCompressionQuality] doubleValue];
    }
    properties[(__bridge NSString *)kCGImageDestinationLossyCompressionQuality] = @(compressionQuality);
    CGColorRef backgroundColor = [options[TXAdImageCoderEncodeBackgroundColor] CGColor];
    if (backgroundColor) {
        properties[(__bridge NSString *)kCGImageDestinationBackgroundColor] = (__bridge id)(backgroundColor);
    }
    CGSize maxPixelSize = CGSizeZero;
    NSValue *maxPixelSizeValue = options[TXAdImageCoderEncodeMaxPixelSize];
    ...
    NSUInteger maxFileSize = [options[TXAdImageCoderEncodeMaxFileSize] unsignedIntegerValue];
    if (maxFileSize > 0) {
        properties[kTXAdCGImageDestinationRequestedFileSize] = @(maxFileSize);
    ...
    if (options[TXAdImageCoderEncodeEmbedThumbnail]) {
        embedThumbnail = [options[TXAdImageCoderEncodeEmbedThumbnail] boolValue];
    }
    properties[(__bridge NSString *)kCGImageDestinationEmbedThumbnail] = @(embedThumbnail);
    
    BOOL encodeFirstFrame = [options[TXAdImageCoderEncodeFirstFrameOnly] boolValue];
...
#pragma mark - TXAdAnimatedImageCoder
...
- (BOOL)scanAndCheckFramesValidWithImageSource:(CGImageSourceRef)imageSource {
    ...
    NSMutableArray<TXAdImageIOCoderFrame *> *frames = [NSMutableArray arrayWithCapacity:frameCount];
    for (size_t i = 0; i < frameCount; i++) {
        TXAdImageIOCoderFrame *frame = [[TXAdImageIOCoderFrame alloc] init];
        frame.index = i;
