 ...
 */
@interface TXAdImageCacheConfig : NSObject <NSCopying>

/**
 Gets the default cache config used for shared instance or initialization when it does not provide any cache config. Such as `TXAdImageCache.sharedImageCache`.
 ...
 */
@property (nonatomic, class, readonly, nonnull) TXAdImageCacheConfig *defaultCacheConfig;
...
/*
 * The option to control weak memory cache for images. When enable, `TXAdImageCache`'s memory cache will use a weak maptable to store the image at the same time when it stored to memory, and get removed at the same time.
 ...
 * When enabling this option, we will sync back the image from weak maptable to strong cache during next time top level `txad_setImage` function call.
 ...
 */
@property (assign, nonatomic) TXAdImageCacheConfigExpireType diskCacheExpireType;
...
/**
 * The dispatch queue attr for ioQueue. You can config the QoS and concurrent/serial to internal IO queue. The ioQueue is used by TXAdImageCache to access read/write for disk data.
...
/**
 * The custom memory cache class. Provided class instance must conform to `TXAdMemoryCache` protocol to allow usage.
 * Defaults to built-in `TXAdMemoryCache` class.
 * @note This value does not support dynamic changes. Which means further modification on this value after cache initialized has no effect.
 */
@property (assign, nonatomic, nonnull) Class memoryCacheClass;

/**
 * The custom disk cache class. Provided class instance must conform to `TXAdDiskCache` protocol to allow usage.
 * Defaults to built-in `TXAdDiskCache` class.
 * @note This value does not support dynamic changes. Which means further modification on this value after cache initialized has no effect.
 */
