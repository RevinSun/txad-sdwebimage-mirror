...
@interface TXAdImageFramePool ()

@property (class, readonly) NSMapTable *providerFramePoolMap;

@property (weak) id<TXAdAnimatedImageProvider> provider;
...
TXAd_LOCK_DECLARE_STATIC(_providerFramePoolMapLock);

@implementation TXAdImageFramePool
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _fetchQueue.name = @"com.hackemist.TXAdImageFramePool.fetchQueue";
#if TXAd_UIKIT
...
+ (void)initialize {
    // Lock to ensure atomic behavior
    TXAd_LOCK_INIT(_providerFramePoolMapLock);
}

+ (instancetype)registerProvider:(id<TXAdAnimatedImageProvider>)provider {
    // Lock to ensure atomic behavior
    TXAd_LOCK(_providerFramePoolMapLock);
    TXAdImageFramePool *framePool = [self.providerFramePoolMap objectForKey:provider];
    if (!framePool) {
        framePool = [[TXAdImageFramePool alloc] init];
        framePool.provider = provider;
        [self.providerFramePoolMap setObject:framePool forKey:provider];
    }
    framePool.registerCount += 1;
    TXAd_UNLOCK(_providerFramePoolMapLock);
    return framePool;
}

+ (void)unregisterProvider:(id<TXAdAnimatedImageProvider>)provider {
    // Lock to ensure atomic behavior
    TXAd_LOCK(_providerFramePoolMapLock);
    TXAdImageFramePool *framePool = [self.providerFramePoolMap objectForKey:provider];
    if (!framePool) {
...
- (void)prefetchFrameAtIndex:(NSUInteger)index {
    ...
    if (self.fetchQueue.operationCount == 0) {
        ...
        id<TXAdAnimatedImageProvider> animatedProvider = self.provider;
        @weakify(self);
