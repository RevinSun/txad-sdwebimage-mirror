...
@implementation TXAdImageCachesManagerOperation {
    TXAd_LOCK_DECLARE(_pendingCountLock);
}
...
- (instancetype)init {
    if (self = [super init]) {
        TXAd_LOCK_INIT(_pendingCountLock);
        _pendingCount = 0;
    }
    return self;
}
...
- (NSUInteger)pendingCount {
    TXAd_LOCK(_pendingCountLock);
    NSUInteger pendingCount = _pendingCount;
    TXAd_UNLOCK(_pendingCountLock);
    return pendingCount;
}
