...
static NSArray *TXAdBundlePreferredScales(void) {
...
@implementation TXAdImageAssetManager {
    TXAd_LOCK_DECLARE(_lock);
}

+ (instancetype)sharedAssetManager {
    static dispatch_once_t onceToken;
    static TXAdImageAssetManager *assetManager;
    dispatch_once(&onceToken, ^{
        assetManager = [[TXAdImageAssetManager alloc] init];
    });
    return assetManager;
}
...
- (NSString *)getPathForName:(NSString *)name bundle:(NSBundle *)bundle preferredScale:(CGFloat *)scale {
    ...
    NSArray *scales = TXAdBundlePreferredScales();
