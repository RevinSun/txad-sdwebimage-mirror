...
/// Image Cache Options
typedef NS_OPTIONS(NSUInteger, TXAdImageCacheOptions) {
    /**
     * By default, we do not query image data when the image is already cached in memory. This mask can force to query image data at the same time. However, this query is asynchronously unless you specify `TXAdImageCacheQueryMemoryDataSync`
     */
    TXAdImageCacheQueryMemoryData = 1 << 0,
    /**
     * By default, when you only specify `TXAdImageCacheQueryMemoryData`, we query the memory image data asynchronously. Combined this mask as well to query the memory image data synchronously.
     */
    TXAdImageCacheQueryMemoryDataSync = 1 << 1,
    /**
     ...
     */
    TXAdImageCacheQueryDiskDataSync = 1 << 2,
    /**
     * By default, images are decoded respecting their original size. On iOS, this flag will scale down the
     * images to a size compatible with the constrained memory of devices.
     */
    TXAdImageCacheScaleDownLargeImages = 1 << 3,
    /**
     ...
     */
    TXAdImageCacheAvoidDecodeImage API_DEPRECATED("Use TXAdWebImageContextImageForceDecodePolicy instead", macos(10.10, 10.10), ios(8.0, 8.0), tvos(9.0, 9.0), watchos(2.0, 2.0)) = 1 << 4,
    /**
     * By default, we decode the animated image. This flag can force decode the first frame only and produce the static image.
     */
    TXAdImageCacheDecodeFirstFrameOnly = 1 << 5,
    /**
     * By default, for `TXAdAnimatedImage`, we decode the animated image frame during rendering to reduce memory usage. This flag actually trigger `preloadAllAnimatedImageFrames = YES` after image load from disk cache
     */
    TXAdImageCachePreloadAllFrames = 1 << 6,
    /**
     ...
     * Using this option, can ensure we always produce image with your provided class. If failed, an error with code `TXAdWebImageErrorBadImageData` will be used.
     * Note this options is not compatible with `TXAdImageCacheDecodeFirstFrameOnly`, which always produce a UIImage/NSImage.
     */
    TXAdImageCacheMatchAnimatedImageClass = 1 << 7,
};

/**
 *  A token associated with each cache query. Can be used to cancel a cache query
 */
@interface TXAdImageCacheToken : NSObject <TXAdWebImageOperation>
...
/**
 * TXAdImageCache maintains a memory cache and a disk cache. Disk cache write operations are performed
 * asynchronous so it doesn’t add unnecessary latency to the UI.
 */
@interface TXAdImageCache : NSObject
 ...
 */
@property (nonatomic, copy, nonnull, readonly) TXAdImageCacheConfig *config;

/**
 * The memory cache implementation object used for current image cache.
 * By default we use `TXAdMemoryCache` class, you can also use this to call your own implementation class method.
 * @note To customize this class, check `TXAdImageCacheConfig.memoryCacheClass` property.
 */
@property (nonatomic, strong, readonly, nonnull) id<TXAdMemoryCache> memoryCache;

/**
 * The disk cache implementation object used for current image cache.
 * By default we use `TXAdMemoryCache` class, you can also use this to call your own implementation class method.
 * @note To customize this class, check `TXAdImageCacheConfig.diskCacheClass` property.
 ...
 */
@property (nonatomic, strong, readonly, nonnull) id<TXAdDiskCache> diskCache;
 ...
 */
@property (nonatomic, copy, nullable) TXAdImageCacheAdditionalCachePathBlock additionalCachePathBlock;
 ...
 */
@property (nonatomic, class, readonly, nonnull) TXAdImageCache *sharedImageCache;

/**
 * Control the default disk cache directory. This will effect all the TXAdImageCache instance created after modification, even for shared image cache.
 ...
 * @note If you pass nil, the value will be reset to `~/Library/Caches/com.hackemist.TXAdImageCache`.
 * @note We still preserve the `namespace` arg, which means, if you change this property into `/path/to/use`,  the `TXAdImageCache.sharedImageCache.diskCachePath` should be `/path/to/use/default` because shared image cache use `default` as namespace.
 * Defaults to nil.
 */
 ...
 * The final disk cache directory should looks like ($directory/$namespace). And the default config of shared cache, should result in (~/Library/Caches/com.hackemist.TXAdImageCache/default/)
 *
...
- (nonnull instancetype)initWithNamespace:(nonnull NSString *)ns
                       diskCacheDirectory:(nullable NSString *)directory
                                   config:(nullable TXAdImageCacheConfig *)config NS_DESIGNATED_INITIALIZER;
 ...
 * @param completionBlock A block executed after the operation is finished
 * @note If no image data is provided and encode to disk, we will try to detect the image format (using either `txad_imageFormat` or `TXAdAnimatedImage` protocol method) and animation status, to choose the best matched format, including GIF, JPEG or PNG.
 */
- (void)storeImage:(nullable UIImage *)image
            forKey:(nullable NSString *)key
            toDisk:(BOOL)toDisk
 ...
 *  @note the completion block will be always executed on the main queue
 */
- (void)diskImageDataQueryForKey:(nullable NSString *)key completion:(nullable TXAdImageCacheQueryDataCompletionBlock)completionBlock;

/**
 * Asynchronously queries the cache with operation and call the completion when done.
 *
 * @param key       The unique key used to store the wanted image. If you want transformed or thumbnail image, calculate the key with `TXAdTransformedKeyForKey`, `TXAdThumbnailedKeyForKey`, or generate the cache key from url with `cacheKeyForURL:context:`.
 * @param doneBlock The completion block. Will not get called if the operation is cancelled
 *
 * @return a TXAdImageCacheToken instance containing the cache operation, will callback immediately when cancelled
 */
- (nullable TXAdImageCacheToken *)queryCacheOperationForKey:(nullable NSString *)key done:(nullable TXAdImageCacheQueryCompletionBlock)doneBlock;
 ...
 */
- (nullable TXAdImageCacheToken *)queryCacheOperationForKey:(nullable NSString *)key options:(TXAdImageCacheOptions)options done:(nullable TXAdImageCacheQueryCompletionBlock)doneBlock;
 ...
 * @param doneBlock The completion block. Will not get called if the operation is cancelled
 *
 * @return a TXAdImageCacheToken instance containing the cache operation, will callback immediately when cancellederation, will callback immediately when cancelled
 * @warning If you query with thumbnail cache key, you'd better not pass the thumbnail pixel size context, which is undefined behavior.
 */
...
/**
 * TXAdImageCache is the built-in image cache implementation for web image manager. It adopts `TXAdImageCache` protocol to provide the function for web image manager to use for image loading process.
 */
@interface TXAdImageCache (TXAdImageCache) <TXAdImageCache>
