...
typedef NSData * _Nullable (^TXAdWebImageDownloaderDecryptorBlock)(NSData * _Nonnull data, NSURLResponse * _Nullable response);
...
*/
@protocol TXAdWebImageDownloaderDecryptor <NSObject>
...
/// @note If nil is returned, the image download will be marked as failed with error `TXAdWebImageErrorBadImageData`
...
*/
@interface TXAdWebImageDownloaderDecryptor : NSObject <TXAdWebImageDownloaderDecryptor>

/// Create the data decryptor with block
/// @param block A block to control decrypt logic
- (nonnull instancetype)initWithBlock:(nonnull TXAdWebImageDownloaderDecryptorBlock)block;

/// Create the data decryptor with block
/// @param block A block to control decrypt logic
+ (nonnull instancetype)decryptorWithBlock:(nonnull TXAdWebImageDownloaderDecryptorBlock)block;
...
@interface TXAdWebImageDownloaderDecryptor (Conveniences)

/// Base64 Encoded image data decryptor
@property (class, readonly, nonnull) TXAdWebImageDownloaderDecryptor *base64Decryptor;
