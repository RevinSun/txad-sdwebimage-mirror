...
/**
 ...
 Call `setImage:` with `UIImage(NSImage)` which conforms to `TXAdAnimatedImage` protocol will start animated image rendering. Call with normal UIImage(NSImage) will back to normal UIImageView(NSImageView) rendering
 ...
 */
NS_SWIFT_UI_ACTOR
@interface TXAdAnimatedImageView : UIImageView
/**
 ...
 @warning Pay attention if you directly update the player's property like `totalFrameCount`, `totalLoopCount`, the same property on `TXAdAnimatedImageView` may not get synced.
 */
@property (nonatomic, strong, readonly, nullable) TXAdAnimatedImagePlayer *player;
...
/// Asynchronous setup animation playback mode. Default mode is TXAdAnimatedImagePlaybackModeNormal.
@property (nonatomic, assign) TXAdAnimatedImagePlaybackMode playbackMode;
...
/**
 ...
 @note The best practice to use incremental load is using `initWithAnimatedCoder:scale:` in `TXAdAnimatedImage` with animated coder which conform to `TXAdProgressiveImageCoder` as well. Then call incremental update and incremental decode method to produce the image.
...
/**
 If the image which conforms to `TXAdAnimatedImage` protocol has more than one frame, set this value to `YES` will automatically
