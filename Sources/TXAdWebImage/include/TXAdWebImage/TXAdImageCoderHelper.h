...
typedef NS_ENUM(NSUInteger, TXAdImageCoderDecodeSolution) {
    /// automatically choose the solution based on image format, hardware, OS version. This keep balance for compatibility and performance. Default after TXAdWebImage 5.13.0
    TXAdImageCoderDecodeSolutionAutomatic,
    /// always use CoreGraphics to draw on bitmap context and trigger decode. Best compatibility. Default before TXAdWebImage 5.13.0
    TXAdImageCoderDecodeSolutionCoreGraphics,
    ...
    TXAdImageCoderDecodeSolutionUIKit
};
...
/// The downside is that force-decode may consume RAM and CPU, and may loss the `lazy` support (lazy CGImage can be purged when memory warning, and re-created if need), see more: `TXAdImageCoderDecodeUseLazyDecoding`
typedef NS_ENUM(NSUInteger, TXAdImageForceDecodePolicy) {
    ...
    TXAdImageForceDecodePolicyAutomatic,
    /// Never force decode input CGImage
    TXAdImageForceDecodePolicyNever,
    /// Always force decode input CGImage (only once)
    TXAdImageForceDecodePolicyAlways
};
 ...
 */
+ (UIImage * _Nullable)animatedImageWithFrames:(NSArray<TXAdImageFrame *> * _Nullable)frames;
 ...
 */
+ (NSArray<TXAdImageFrame *> * _Nullable)framesFromAnimatedImage:(UIImage * _Nullable)animatedImage NS_SWIFT_NAME(frames(from:));
...
/// Calculate the limited image size with the bytes, when using `TXAdImageCoderDecodeScaleDownLimitBytes`. This preserve aspect ratio and never scale up
/// @param imageSize The image size (in pixel or point defined by caller)
...
/**
 ...
 @return The decoded image
 */
+ (UIImage * _Nullable)decodedImageWithImage:(UIImage * _Nullable)image policy:(TXAdImageForceDecodePolicy)policy;
...
/**
 ...
 @return The decoded and probably scaled down image
 */
+ (UIImage * _Nullable)decodedAndScaledDownImageWithImage:(UIImage * _Nullable)image limitBytes:(NSUInteger)bytes policy:(TXAdImageForceDecodePolicy)policy;

/**
 Control the default force decode solution. Available solutions  in `TXAdImageCoderDecodeSolution`.
 @note Defaults to `TXAdImageCoderDecodeSolutionAutomatic`, which prefers to use UIKit for JPEG/HEIF, and fallback on CoreGraphics. If you want control on your hand, set the other solution.
 */
@property (class, readwrite) TXAdImageCoderDecodeSolution defaultDecodeSolution;
