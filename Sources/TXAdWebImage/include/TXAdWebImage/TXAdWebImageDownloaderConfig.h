...
typedef NS_ENUM(NSInteger, TXAdWebImageDownloaderExecutionOrder) {
    /**
     * Default value. All download operations will execute in queue style (first-in-first-out).
     */
    TXAdWebImageDownloaderFIFOExecutionOrder,
    
    /**
     * All download operations will execute in stack style (last-in-first-out).
     */
    TXAdWebImageDownloaderLIFOExecutionOrder
};
 ...
 */
@interface TXAdWebImageDownloaderConfig : NSObject <NSCopying>

/**
 Gets the default downloader config used for shared instance or initialization when it does not provide any downloader config. Such as `TXAdWebImageDownloader.sharedDownloader`.
 ...
 */
@property (nonatomic, class, readonly, nonnull) TXAdWebImageDownloaderConfig *defaultDownloaderConfig;
...
/**
 * Gets/Sets a subclass of `TXAdWebImageDownloaderOperation` as the default
 * `NSOperation` to be used each time TXAdWebImage constructs a request
 * operation to download an image.
 * Defaults to nil.
 * @note Passing `NSOperation<TXAdWebImageDownloaderOperation>` to set as default. Passing `nil` will revert to `TXAdWebImageDownloaderOperation`.
 */
@property (nonatomic, assign, nullable) Class operationClass;

/**
 * Changes download operations execution order.
 * Defaults to `TXAdWebImageDownloaderFIFOExecutionOrder`.
 */
@property (nonatomic, assign) TXAdWebImageDownloaderExecutionOrder executionOrder;
 ...
 * For example, if we config [200, 400) but server response is 503, the download will fail with error code `TXAdWebImageErrorInvalidDownloadStatusCode`.
 ...
 * For example, if we config ["image/png"] but server response is "application/json", the download will fail with error code `TXAdWebImageErrorInvalidDownloadContentType`.
